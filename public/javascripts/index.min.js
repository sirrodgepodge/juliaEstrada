...
function calcTriggerPoints() {
    titleTop = Math.ceil($landing.outerHeight()), titleHeight = $title.height(), landingHeadFixPoint = titleTop - Math.ceil($landingHead.position().top), 
    contactTop = .92 * Math.ceil($contact.offset().top), downAnimReached = .395 * titleTop + 4.5, 
    landingScroll();
}
