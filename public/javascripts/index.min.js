...
function calcTriggerPoints() {
    ...
    landingHeadFixPoint = titleTop - Math.ceil($landingHead.position().top), contactTop = .92 * Math.ceil($contact.offset().top), 
    downAnimReached = .395 * titleTop + 4.5, landingScroll();
}
...
var landingTogglerClicked = !1, main = function() {
    ...
    for (i = 1; totalSlices + 1 > i; i++) struct += '<div class="slice total-slices-' + totalSlices + " slice-" + i + '" style="background-image: url(), linear-gradient(top, rgba(0,0,0,' + Math.pow(2, i - totalSlices - 2) + ") 0%,rgba(0,0,0, " + (Math.pow(2, i - totalSlices) - 1) + ') 100%); background-image: url(), -webkit-linear-gradient(top, rgba(0,0,0,0) 0%,rgba(0,0,0, .7) 100%);">';
    for (i = totalSlices; i > 0; i--) struct += "</div>";
    $floatFold.each(function() {
        var $this = $(this), imgPath = $this.attr("src"), classes = $this.attr("class");
        $this.replaceWith($("<div class=" + classes + "></div>").append($(struct.replace(floatFoldImgRegex, "background-image: url(" + imgPath + ")"))));
...
}, titleTop, titleHeight, landingHeadFixPoint, contactTop, downAnimReached, pagePos = 0, titleFixed = !1, landingHeadFixed = !1, topTextShowing = !0, contactPopped = !1;
